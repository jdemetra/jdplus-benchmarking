 ...
 * @author Jean Palate
 */
public class CubicSplineDocument extends AbstractMultiTsDocument<CubicSplineSpec, BenchmarkingResults> {

    public CubicSplineDocument() {
        super(CubicSplineSpec.DEFAULT);
    }

    @Override
    protected BenchmarkingResults internalProcess(CubicSplineSpec spec, List<TsData> data) {
        if (data.isEmpty())
            return null;
        ...
        if (high == null)
            return null;
        TsData benchmark = CubicSplineProcessor.PROCESSOR.benchmark(high, low, spec);
        if (benchmark == null)
            return null;
