package jdplus.benchmarking.desktop.plugin;
...
public class BenchmarkingRuntimeDependenciesTest {

    @Test
    public void test() throws IOException {
        assertThat(getRuntimeDependencies(BenchmarkingRuntimeDependenciesTest.class))
                .describedAs("Check runtime dependencies")
                .satisfies(BenchmarkingRuntimeDependenciesTest::checkBenchmarking)
                .hasSize(2);
    }

    private static void checkBenchmarking(List<? extends GAV> coordinates) {
        assertThatGroupId(coordinates, "eu.europa.ec.joinup.sat")
                .has(sameVersion())
                .extracting(GAV::getArtifactId)
                .are(matchingPattern(compile("^jdplus-benchmarking-base-\\w+$")))
                .hasSize(2);
    }
